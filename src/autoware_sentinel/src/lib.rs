...
fn run() -> Result<(), NodeError> {
    ...
    let mut executor = Executor::<_, 16, 8192>::open(&config)?;

    let _node = executor.create_node("sentinel")?;

    info!("Executor ready — {} handle slots", 16);

    // Phase 6.3–6.6 will register subscriptions, timers, and publishers here.
