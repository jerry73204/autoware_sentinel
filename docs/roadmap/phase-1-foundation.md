...
`cargo nano-ros generate`, then ports three small, stateless packages to prove the
pipeline works and to establish project conventions (per-package directory layout, message
generation, testing).
...
generated per-package into a local `generated/` directory. The `.cargo/config.toml` is
auto-generated by `cargo nano-ros generate` and patches crates.io entries to point at the
local `generated/` crates and the nano-ros checkout.

```
src/autoware_stop_filter/
├── Cargo.toml                 # version deps, no path deps
├── .cargo/config.toml         # [patch.crates-io] — auto-generated
├── package.xml                # ROS message dependencies
├── justfile                   # generate, build, test, clean recipes
...
Message dependencies use `version = "*"` (resolved via `[patch.crates-io]` to generated
crates). No `path` dependencies, no workspace inheritance.
...
### .cargo/config.toml pattern

Auto-generated by `cargo nano-ros generate`. Patches nano-ros core crates and generated
message crates:
...
nav_msgs = { path = "generated/nav_msgs" }
...
Declares message dependencies for `cargo nano-ros generate`:
...
<package format="3">
  ...
  <description>Velocity stop filter for Autoware</description>
...
clean:
    rm -rf generated .cargo/config.toml target
```
...
A root `justfile` defines a `packages` list and provides convenience recipes:

```just
packages := "autoware_stop_filter autoware_vehicle_velocity_converter autoware_shift_decider"

generate-bindings:  # sources scripts/activate_autoware.sh, runs codegen per-package
build:              # cargo build in each package
test:               # cargo test in each package
format / format-check:  # cargo fmt on each package
cross-check:        # cargo build --target thumbv7em-none-eabihf in each
...
- [x] Generate message crates via `cargo nano-ros generate`
...
### 1.5 — Revision: Standalone packages

Migrate from root Cargo workspace to standalone per-package builds following the nano-ros
convention (see [Per-Package Structure](#per-package-structure) above).

- [x] Create `package.xml` for each algorithm crate (3 files)
- [x] Create per-package `justfile` (3 files)
- [x] Rewrite each algorithm crate `Cargo.toml` — remove workspace inheritance, use `version = "*"` for message deps
- [x] Generate messages per-package: `just generate` in each crate
- [x] Remove root workspace `Cargo.toml`
- [x] Remove shared `src/messages/` directory
- [x] Create root `justfile` with convenience recipes (`generate-bindings`, `build`, `test`, `cross-check`, `ci`)
- [x] Verify: `cargo build` + `cargo test` in each package, `cargo build --target thumbv7em-none-eabihf`
...
- [x] Each package builds independently with per-package `just build` / `just test`.
- [x] Root workspace `Cargo.toml` and shared `src/messages/` removed.
